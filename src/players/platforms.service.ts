...
export class PlatformsService {
    ...
    async getPlatformById(id:number) : Promise<Platform>{
        const found = await this.platformsrepository.findOne({where:{id}});
    ...
    async createPlatform(createPlatformDto : CreatePlatformDto,image:any, user: User) : Promise<Platform>{
     ...
     async UpdatePlatformById(id:number,createPlatformDto : CreatePlatformDto,image:any,user:User) : Promise<Platform>{
        const platform: Platform = await this.getPlatformById(id);
